...
export default function PricingPage() {
  ...
  const handleSubscribe = async (plan) => {
    ...
    //   Example API call – replace with your actual endpoint
      const response = await axios.post("/api/create-order", { plan });
  ...
  const initiateRazorpay = (orderId, amount, currency) => {
    // TODO: Implement Razorpay checkout integration here
    console.log("Initiating Razorpay:", { orderId, amount, currency });
  };
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          ...
          <div className="group bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-8 hover:border-teal-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-teal-500/10 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
            ...
            <button
              ...
              className="mt-8 w-full py-4 rounded-xl text-sm font-semibold tracking-wide transition-all duration-200 flex items-center justify-center gap-2 bg-white/5 border border-white/10 hover:bg-white/10 text-white disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading === "free" ? (
                <>
